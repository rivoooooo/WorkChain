...
const privacyCopy: Record<Language, PrivacyCopy> = {
  zh: {
    ...
    sections: [
      ...
      { title: 'VibeLoft 遥测', body: '本站使用 VibeLoft 了解页面运行与产品使用情况。遥测请求由浏览器直接发送到 VibeLoft API，不会写入 WorkChain 或 Supabase 业务数据库，也不会与本站账号关联，因为本站不提供账号系统。VibeLoft 对接收数据的处理受其自身服务政策约束。' },
  ...
  'zh-tw': {
    ...
    sections: [
      ...
      { title: 'VibeLoft 遙測', body: '本站使用 VibeLoft 瞭解頁面運作與產品使用情況。遙測請求由瀏覽器直接傳送至 VibeLoft API，不會寫入 WorkChain 或 Supabase 業務資料庫，也不會與本站帳號關聯，因為本站沒有帳號系統。VibeLoft 對接收資料的處理受其服務政策約束。' },
  ...
  en: {
    ...
    sections: [
      ...
      { title: 'VibeLoft telemetry', body: 'We use VibeLoft to understand page operation and product usage. Telemetry requests are sent directly from the browser to the VibeLoft API and are not written to the WorkChain or Supabase application database. They cannot be linked to a WorkChain account because this site has no account system. VibeLoft handles received data under its own service policies.' },
  ...
  ja: {
    ...
    sections: [
      ...
      { title: 'VibeLoftテレメトリ', body: 'ページの動作と製品利用状況を把握するためVibeLoftを使用します。テレメトリはブラウザからVibeLoft APIへ直接送られ、WorkChainまたはSupabaseの業務データベースには書き込まれません。本サイトにはアカウント機能がないため、サイト内アカウントとも関連付けられません。受信データはVibeLoft自身の方針に従って処理されます。' },
  ...
  ar: {
    ...
    sections: [
      ...
      { title: 'قياس الاستخدام عبر VibeLoft', body: 'نستخدم VibeLoft لفهم تشغيل الصفحات واستخدام المنتج. تُرسل طلبات القياس مباشرة من المتصفح إلى واجهة VibeLoft ولا تُكتب في قاعدة بيانات WorkChain أو Supabase. ولا ترتبط بحساب في الموقع لأن الموقع لا يوفر نظام حسابات. تعالج VibeLoft البيانات المستلمة وفق سياساتها الخاصة.' },
  ...
  hi: {
    ...
    sections: [
      ...
      { title: 'VibeLoft टेलीमेट्री', body: 'पेज संचालन और उत्पाद उपयोग समझने के लिए VibeLoft का उपयोग होता है। टेलीमेट्री अनुरोध ब्राउज़र से सीधे VibeLoft API को जाते हैं और WorkChain या Supabase के ऐप डेटाबेस में नहीं लिखे जाते। साइट पर खाता प्रणाली नहीं है, इसलिए ये किसी WorkChain खाते से नहीं जुड़ते। प्राप्त डेटा का प्रबंधन VibeLoft की अपनी नीतियों के अधीन है।' },
  ...
  tr: {
    ...
    sections: [
      ...
      { title: 'VibeLoft telemetrisi', body: 'Sayfa çalışmasını ve ürün kullanımını anlamak için VibeLoft kullanılır. Telemetri istekleri tarayıcıdan doğrudan VibeLoft API’ye gider; WorkChain veya Supabase uygulama veritabanına yazılmaz. Sitede hesap sistemi olmadığı için bir WorkChain hesabıyla ilişkilendirilmez. Alınan veriler VibeLoft’un kendi hizmet politikalarına tabidir.' },
  ...
  es: {
    ...
    sections: [
      ...
      { title: 'Telemetría de VibeLoft', body: 'Usamos VibeLoft para conocer el funcionamiento de las páginas y el uso del producto. Las solicitudes se envían directamente desde el navegador a la API de VibeLoft y no se escriben en la base de datos de WorkChain ni de Supabase. No se vinculan a una cuenta porque el sitio no tiene sistema de cuentas. VibeLoft trata los datos recibidos según sus propias políticas.' },
  ...
  bo: {
    ...
    sections: [
      ...
      { title: 'VibeLoft རྒྱང་འཇལ།', body: 'ཤོག་ངོས་དང་ཐོན་རྫས་བེད་སྤྱོད་ཤེས་ཆེད VibeLoft བེད་སྤྱོད། རྒྱང་འཇལ་རེ་ཞུ་དྲ་བཤེར་ནས VibeLoft API ལ་ཐད་ཀར་བསྐུར་ཞིང WorkChain དང Supabase གཞི་གྲངས་མཛོད་དུ་མི་འཇུག ཐོ་གཞུང་མ་ལག་མེད་པས་ཐོ་གཞུང་དང་མི་སྦྲེལ།' },
